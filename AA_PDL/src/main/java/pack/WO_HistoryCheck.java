...
public class WO_HistoryCheck extends AA_PDL{
	
	
	public static void Prenote_deposit_History(String SSN,String FileName) throws Exception
	{
			...
			if(SSN.equals(RegSSN))
			{
				...
				for (int i = 2; i <= schsize; i++) {
					...
					//for (int j = 2; j <= schsize1; j++) {
						...
						int j=0;
						if((field).contains("write"))
						{
							test.log(LogStatus.PASS,"Write Off Record is  Displayed and the transaction displayed is ::"+field );
		/*					j=j+1;
							test.log(LogStatus.PASS,"Write Off Records displayed ::"+j );*/
	
							break;
						}
						else
						{
							test.log(LogStatus.PASS,"WriteOff Record is not Displayed" );
